# Blue Glass Design System design system
...
workspace package (`@workspace/blue-glass-design-system`): other artifacts depend
on it and import its theme and components directly.

## Visual direction

- Dark mode is the hero experience: near-black ocean backgrounds, navy layers,
  cool white type, and hairline blue-grey borders.
- Electric blue is reserved for the clearest action, active selection, focus
  ring, and concise status emphasis. Do not make every interactive item blue.
- Build depth with adjacent navy values, translucency, blur, and an inner
  highlight before reaching for large drop shadows.
- Interactive glass should feel physical: use a restrained moving perimeter
  highlight, a single passing sheen, and a small lift on hover with a grounded
  press state. Respect reduced-motion preferences.
- Large surfaces use generous radii; compact controls stay tighter. Glass should
  feel engineered and calm, never bubbly or decorative.
- The default applied composition is a dense pricing comparison: centered
  heading, compact segmented control, equal-height plan cards, a visually
  selected recommendation, and full-width actions.
- Reference sources are retained in `docs/references/`: `product-palette.png`
  supplies the blue family and `pricing-layout.png` supplies component hierarchy.
...
`@workspace/blue-glass-design-system/components/...`. Never use a `@/` alias inside
...
for `@workspace/blue-glass-design-system` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/blue-glass-design-system` as a `workspace:*` dependency,
...
- Building or styling web UI: `artifacts/blue-glass-design-system/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/blue-glass-design-system/docs/consuming-expo.md`
- Building or styling a slide deck: `artifacts/blue-glass-design-system/docs/consuming-slides.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/blue-glass-design-system/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/blue-glass-design-system/docs/migrating-expo.md`
