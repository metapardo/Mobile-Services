...
> **Documented exception — not scope creep, don't "fix" it:** `artifacts/detail-hub/api/[...path].ts` does `export { default } from "@workspace/api-server";`, importing backend-engineer's Express app directly as a serverless entry point. This exists because `api-server` deploys as a Vercel Serverless Function under the *same* Vercel project as `detail-hub` (Root Directory `artifacts/detail-hub`), for same-origin cookies — Better Auth's session cookie can't cross domains cleanly otherwise, and a cross-origin `SameSite=None; Secure` cookie workaround would be more fragile than just being same-origin. Vercel's filesystem routing turns `<Root Directory>/api/*` into serverless functions; `[...path].ts` is Vercel's catch-all naming convention (an empty-name catch-all, `[...].ts`, is not valid — Vercel requires an identifier in the brackets, same as Next.js). It is exactly one file, owned and maintained by frontend-engineer, not a general license for `detail-hub` to reach into `api-server` code elsewhere or vice versa. What makes this resolve: `artifacts/api-server/package.json` has an `"exports"` field (`{ ".": "./src/app.ts" }`) exposing the default-exported `Express` app — the same TS-source-via-`exports` pattern `lib/db` and `lib/api-client-react` already use, consumed here via a plain `workspace:*` dependency in `detail-hub/package.json`. `detail-hub/tsconfig.json`'s `include` was widened to `["src/**/*", "api/**/*"]` so this file is actually typechecked (it lives outside `src/`, and would otherwise be silently skipped). If backend-engineer restructures how `app.ts` builds/exports the app, this breaks unless the export shape (a default-exported `Express` instance) is kept intact.
