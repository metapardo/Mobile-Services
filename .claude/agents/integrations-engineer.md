...
**Two separate Stripe integrations — don't conflate them.** (1) The tenant-facing payment work above is *a tenant's own clients* paying *them* for detailing services — that money never touches your (RareAir's) Stripe account. (2) **Platform subscription billing** is the opposite direction: RareAir charging each tenant organization a recurring monthly fee to use the app, via Stripe Billing/Subscriptions and a Stripe Checkout Session in subscription mode (`trial_period_days: 14`, single flat price, using the Price ID from Stripe's dashboard — that Price is created manually in the dashboard, not in code). Use Stripe's *hosted* Checkout and Customer Portal for this — don't build custom subscription-management UI. This needs its own webhook endpoint (`checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`, `invoice.payment_failed`), separate from the tenant-charge webhook, driving a new `organizationBilling` table's `subscriptionStatus`. Real signature verification on this webhook is just as non-negotiable as the tenant-payment one.

**Open, unresolved question — don't guess at it:** now that the app is multi-tenant, does each tenant need their own connected Stripe account (Stripe Connect) to receive their own clients' payments, or does the tenant-facing payment flow work some other way? This affects the *tenant-facing* Stripe integration above, not platform billing. Flag it rather than assuming an answer.
