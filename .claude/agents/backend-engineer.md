...
- Don't build payment-processor logic (Stripe/Square calls, webhook signature verification, tokenization) here directly — define the endpoint shape (e.g., `POST /bookings/:id/charge`) and hand the actual processor integration to integrations-engineer.
